...
const Service = (props) => {
  const { service: { name, desc, img, id } } = props
  return (
    <div className='overflow-hidden rounded-md service-box'>
      ...
      <NavLink to={`/service/${id}`} className='transition-tracking transition-color duration-500 text-center pl-3 text-text_dark py-3 flex items-center gap-2'>Make an Appoinment <BsArrowRightCircle /></NavLink>
    </div>
  );
};
